...
int main(int argc, char *argv[]) {
    ...
    printf("%s %d%s", "Frame Store Size =", FRAME_STORE_SIZE, ";");
    printf(" %s %d\n", "Variable Store Size =", VAR_STORE_SIZE);
...
//a function to create an empty directory backing store
//or empty the backing store if it already exists
int make_backingstore() {
    int errCode = 0;
    struct stat st;
    int found = 0;
    ...
    //check if the backing store already exists
    if(stat("backing_store",&st) == 0) {
            //it is found
            found = 1;
    }
    if (found) {
        //remove and empty the directory
        char* cmd = (char*) calloc(1, 10+namelen); 
        strncat(cmd, "rm -r ", 10);
        strncat(cmd, dir, namelen);
        errCode = system(cmd);
        free(cmd);
    }
